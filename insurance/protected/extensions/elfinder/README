...
REQUIREMENTS
  ...
  For instance, one may wish to use PHP's FileInfo extension
  (http://php.net/manual/en/book.fileinfo.php) to check MIME-types of uploaded
  files.
...
USAGE
  ...
  1. `selector` - jQuery selector used to point to container element ElFinder
  must be appended to. This parameter is optional and defaults to `#elfinder`.

  2. `clientOptions` - an array of ElFinder's client configuration options
  described here:
  ...
  with some WYSIWYG editor by setting `getFileCallback`
  ...
  3. `connectorRoute` - Yii route to ElFinder's connector action
  (`admin/fileUploaderConnector`).

  4. `connectorOptions` - an array of ElFinder's connector configuration
  ...
  Some people using `elfinder-widget`
  (http://www.yiiframework.com/extension/elfinder-widget) faced the problem of
...
ADDITIONAL NOTES
  ...
  1). I recommend `ckeditor-integration`
  ...
  CKEditor extension use `filebrowserBrowseUrl`
  (http://docs.cksource.com/ckeditor_api/symbols/CKEDITOR.config.html#.filebrowserBrowseUrl)
  like this:
